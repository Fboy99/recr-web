...
export function Navbar() {
  ...
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const [activeLink, setActiveLink] = useState("");
  const [selectedLanguage, setSelectedLanguage] = useState<Language>(languages.find(lang => lang.code === language) || languages[0]);  // Initialize with context language

  // Load saved language from localStorage on component mount
  useEffect(() => {
    const savedLanguage = localStorage.getItem('language') || language; // Fallback to context language
    i18n.changeLanguage(savedLanguage);
    const selectedLang = languages.find(lang => lang.code === savedLanguage);
    if (selectedLang) setSelectedLanguage(selectedLang);
  }, [i18n, language]); 
  ...
  const handleLanguageChange = (lang: Language) => {
    ...
    i18n.changeLanguage(lang.code); // This changes the language
    localStorage.setItem('language', lang.code); // Save selected language
    setLanguage(lang.code as "en" | "fr" | "de");
    toggleDropdown();
    console.log("Current language:", i18n.language);
  };
